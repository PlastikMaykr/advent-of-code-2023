...
const labelsArr = ['J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A',]
const labels = {};
for (let i = 0; i < labelsArr.length; i++) {
    labels[labelsArr[i]] = i + 1;
}
// console.log(labels);
...
function determineType(cards) {
    ...
    const joking = kinds.has('J');
    if (joking && kinds.size > 1) playJoker(kinds);
    const totals = [...kinds.values()].sort().reverse();
    // console.log({ totals, joking });
...
function playJoker(kinds) {
    const sorted = [...kinds.entries()]
        .sort((a, b) => b[1] - a[1]);
    // console.log(sorted);

    const [firstKind, firstCount] = sorted[0];
    const [secondKind, secondCount] = sorted[1];

    if (firstKind == 'J') {
        kinds.set(secondKind, secondCount + firstCount);
    } else {
        kinds.set(firstKind, kinds.get('J') + firstCount);
    }
